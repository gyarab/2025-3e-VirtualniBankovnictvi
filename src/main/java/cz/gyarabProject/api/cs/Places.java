...
public class Places extends Sender {
    ...
    public PageInfo<ATM> atms(
            ...
            LocalDate openTime, Token token
        ...
        HttpResponse<String> response = send(uri, false, token);
        return mapper.readValue(response.body(), new TypeReference<>() {});
    }

    public PageInfo<ATM> atms(
            String place, String lat, String lng, String radius, Region region, String country, String flag,
            String bankCode, int page, int size, String sort, Detail detail, LocalDate openTime, Token token
    ) throws IOException, InterruptedException {
        return atms(place, lat, null, lng, null, radius, region, country, flag, bankCode,
                page, size, sort, detail, openTime, token);
    }

    public ATM atms(int id, Token token) throws IOException, InterruptedException {
    ...
    public List<String> services(int id, Token token) throws IOException, InterruptedException {
        HttpResponse<String> response = send(
                props.getUriWithEnding(bank(), Property.Environment.SANDBOX, "atm",
    ...
    public ATM.Flag flags(Token token) throws IOException, InterruptedException {
        HttpResponse<String> response = send(
    ...
    public String photos(int id, Token token) throws IOException, InterruptedException {
        HttpResponse<String> response = send(
